...
const API_PUBLIC_KEY_HEX = "04c5c00c4f8d1197cc7c3167c52bf7acb054d722f0ef08dcd7e0883236e0d72a3868d9750cb47fa4619248f3d83f0f662671dadc6e2d31c2f41db0161651c7c076";
export class AnkerSolixClient {
    ...
    async getCurrentStatus(siteId, deviceSn) {
    ...
    async getSiteList() {
        const response = await this.request(ENDPOINTS.siteList, {});
        const list = response.site_list;
        return Array.isArray(list) ? list : [];
    }
    async getSceneInfo(siteId) {
        return this.request(ENDPOINTS.sceneInfo, { site_id: siteId });
    }
    async request(endpoint, body) {
        ...
        if (json.code !== 0) {
            const message = json.msg ?? "Unknown API error";
...
export function extractStatusFromScene(siteId, scene, deviceSn) {
    ...
    const panelInput = toWatts(firstNumber(device.photovoltaic_power, device.input_power, solarbankInfo.total_photovoltaic_power), powerUnit);
    const output = toWatts(firstNumber(device.output_power, solarbankInfo.total_output_power), powerUnit);
    return {
        siteId,
